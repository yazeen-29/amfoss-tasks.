...
async def on_ready():
    ...
    try:
        ...
        print(f"🔄 Synced {len(synced)} commands to guild {GUILD_NUM}")
    except Exception as e:
        print("❌ Sync error:", e)
        traceback.print_exc()
...
async def on_app_command_error(interaction: discord.Interaction, error: Exception):
    ...
    try:
        ...
        if not interaction.response.is_done():
            ...
            await interaction.followup.send("⚠️ An error occurred while processing the command.", ephemeral=True)
    except Exception:
        pass
...
async def lyrics(interaction: discord.Interaction, song: str, artist: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            # non-fatal — fallback to lyrics.ovh
            pass
    ...
    await interaction.followup.send("❌ Lyrics not found (tried LRCLib and lyrics.ovh).")
...
async def track(interaction: discord.Interaction, song: str, artist: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if status == 200 and isinstance(data, dict) and data.get("recordings"):
            ...
            # Build embed
            embed = discord.Embed(title=f"{title} — {artist_name or artist}", color=discord.Color.blue())
    ...
    await interaction.followup.send("❌ Track not found on MusicBrainz.")
...
async def playlist(interaction: discord.Interaction, action: str, song: str = None):
    ...
    if action == "add":
        if not song:
            await interaction.followup.send("❌ You must provide a song to add.")
            return
        playlists[uid].append(song)
        await interaction.followup.send(f"✅ Added **{song}** to your playlist.")
    elif action == "remove":
        if not song:
            await interaction.followup.send("❌ You must provide a song to remove.")
            return
        try:
            playlists[uid].remove(song)
            await interaction.followup.send(f"🗑️ Removed **{song}** from your playlist.")
        except ValueError:
        ...
        if not user_list:
            ...
            await interaction.followup.send("🎶 Your Playlist:\n" + "\n".join(f"- {s}" for s in user_list))
    elif action == "clear":
        playlists[uid] = []
